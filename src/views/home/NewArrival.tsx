  ...
  useEffect(() => {
    const fetchNewArrivals = async () => {
      try {
        setIsLoading(true);
        setError(null);
        // Try new-arrivals endpoint first, fallback to trending if not available
        let response = await fetchPublicWithFallback("/api/products/new-arrivals", env.apiBaseUrl);

        if (!response.ok) {
          // Fallback to trending products if new-arrivals endpoint doesn't exist
          response = await fetchPublicWithFallback("/api/products/trending", env.apiBaseUrl);
        }
      ...
      <div className="container px-4 mx-auto">
        <div className="flex flex-col lg:flex-row justify-between gap-6 lg:items-end mb-10">
          <div className="space-y-3">
            ...
            <h2 className="text-3xl md:text-4xl font-semibold text-foreground">New Arrivals</h2>
            <p className="text-muted-foreground max-w-2xl">
              Discover our newest products just added to the collection.
            </p>
          </div>
