...
type AnalyticsEvent = 
  | { event: "product_view"; product_id: string; sku: string; name: string; variant_id?: number; price: number; currency: string }
  | { event: "media_play"; product_id: string; media_id: number; type: string }
  | { event: "add_to_cart"; product_id: string; variant_id?: number; sku: string; price: number; quantity: number }
  | { event: "begin_checkout"; product_id: string; variant_id?: number; price: number; quantity: number }
  | { event: "add_to_wishlist"; product_id: string }
  | { event: "variant_select"; product_id: string; variant_id: number; attributes: Record<string, any> }
